...
GetAbbreviatedDecisionResult
    [Documentation]    Get Decision with abbreviated results from Policy Xacml PDP
     ...
     Log    Checking policies2 ${jsonResponse['onap.restart.tca']}
     ${policy}=    Get From Dictionary    ${jsonResponse}   onap.restart.tca
     Log    Getting from policy ${policy['type']}
     Should Be Equal As Strings    ${resp.status_code}     200
     Dictionary Should Contain Key    ${resp.json()}    policies
