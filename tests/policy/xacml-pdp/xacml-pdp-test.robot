...
GetAbbreviatedDecisionResult
    [Documentation]    Get Decision with abbreviated results from Policy Xacml PDP
     ...
     Log    Received response from policy ${resp.text}
     ${jsonResponse}   Set Variable    ${resp.json()}
     ${policy}=    Get From Dictionary    ${jsonResponse}   policies
     ${policyList}=    Get From Dictionary    ${policy['onap.restart.tca']}
     Log    Getting from policy ${policyList['type']}
     Should Be Equal As Strings    ${resp.status_code}     200
     Dictionary Should Not Contain Key    ${policyList}    properties
