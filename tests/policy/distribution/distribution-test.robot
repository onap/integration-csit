...
InvokeDistributionAndRunEventOnEngine
     Wait Until Keyword Succeeds    5 min    60 sec    InvokeDistributionUsingFile And RunEventOnApexEngine

*** Keywords ***

InvokeDistributionUsingFile And RunEventOnApexEngine
    Move File    ${SCRIPT_DIR}/config/distribution/csar/sample_csar_with_apex_policy.csar    ${SCRIPT_DIR}/config/distribution/temp/sample_csar_with_apex_policy.csar
    Sleep    30 seconds    "Waiting for the Policy Distribution to call Policy API and PAP"
    ...
    Should Be Equal As Strings    ${resp.status_code}   200
    Move File    ${SCRIPT_DIR}/config/distribution/temp/sample_csar_with_apex_policy.csar    ${SCRIPT_DIR}/config/distribution/csar/sample_csar_with_apex_policy.csar
