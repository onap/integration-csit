...
Trust Merger's Copier successfully backs up files
    ...
    Run Trust Merger And Merge Truststore Files To Pem  ${VALID_ENVS_AND_EXTRA_OPTIONAL_ENVS}  0  ${JKS_KEYSTORE_MOUNT_PATH}  ${JKSBAK_KEYSTORE_EXPECTED_PATH}

Trust Merger's Copier successfully copies file
    [Tags]      OOM-TRUST-STORE-MERGER
    [Documentation]  Run with valid env file and expect successfully copied file
    Run Trust Merger And Merge Truststore Files To Pem  ${VALID_ENVS_AND_EXTRA_OPTIONAL_ENVS}  0  ${JKS_KEYSTORE_MOUNT_PATH}  ${JKS_KEYSTORE_EXPECTED_PATH}
