...
Trust Merger fails when file to copy does not exist
    [Tags]      OOM-TRUST-STORE-MERGER
    [Documentation]  Run with invalid extra env file and expect code 13
    Run Trust Merger And Expect Error  ${INVALID_KEYSTORE_SOURCE_PATHS}  12

Trust Merger fails when only one extra optional env is set
    [Tags]      OOM-TRUST-STORE-MERGER
    [Documentation]  Run with invalid extra env file and expect code 6
    Run Trust Merger And Expect Error  ${INVALID_EMPTY_KEYSTORE_DESTINATION_PATH}  6
