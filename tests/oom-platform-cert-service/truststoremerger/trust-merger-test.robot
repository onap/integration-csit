...
Trust Merger fails when a variable is empty
    [Tags]      OOM-TRUST-STORE-MERGER
    [Documentation]  Run with empty TRUSTSTORES_PASSWORDS env and expect error code
...
Trust Merger fails when aliases are duplicated
    [Tags]      OOM-TRUST-STORE-MERGER
    [Documentation]  Run with duplicated aliases in truststories and expect error code
