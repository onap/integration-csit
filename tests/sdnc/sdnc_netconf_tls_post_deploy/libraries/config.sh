...
CERT_TLS_DIR="${WORKSPACE}"/tests/sdnc/sdnc_netconf_tls_post_deploy/aaf_cert/simulator/*
...
if [[ -n $CONTAINER_ID ]]; then
  if [[ "$CONTAINER_NAME" == "sdnc" ]]; then
    sdnc_conf
  elif [[ "$CONTAINER_NAME" == "netconf-simulator" && "$CONFIG_TLS" != "True" ]]; then
    netconf-simulator_conf
  elif [[ "$CONTAINER_NAME" == "netconf-simulator" && "$CONFIG_TLS" == "True" ]]; then
    CERT_DIR="${CERT_TLS_DIR}"
    netconf-simulator_conf
  fi
fi
