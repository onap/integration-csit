...
class ClientManager:
    ...
    def can_install_keystore_and_truststore_certs(self, cmd, container_name):
        ...
        if container_name == NETCONF_PNP_SIM_CONTAINER_NAME:
            print("Generating PEM files for %s from JKS files: {0}".format(container_name))
    ...
    @staticmethod
    def configure_tls(cmd, cmd_tls, sdnc_container_name, sim_container_name):
        try:
            resp_code_sdnc = subprocess.call(["%s %s %s" % (cmd, sim_container_name, True)], shell=True,
                                             stdout=DEV_NULL, stderr=subprocess.STDOUT)
            resp_code_sim = subprocess.call(["%s %s" % (cmd_tls, sdnc_container_name)], shell=True, stdout=DEV_NULL,
                                            stderr=subprocess.STDOUT)
            print("Response Code from Config_tls.sh execution SDNC: {0}, and for Netconf-Pnp-Simulator: {1}".format(
                resp_code_sdnc, resp_code_sim))
            if not (resp_code_sdnc == 0 and resp_code_sim == 0):
                return False
            return True
        except subprocess.CalledProcessError as e:
            print("CalledProcessError Certificate Installation failed: {0}".format(e))
            return False
