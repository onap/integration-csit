...
Run Cert Service Client Container And Validate Exit Code With Correct JKS File Creation
    ...
    ${canopen}=  Can Open Keystore And Truststore By Pass  ${CLIENT_CONTAINER_NAME}
    Remove Client Container And Save Logs  ${CLIENT_CONTAINER_NAME}
    Should Be Equal As Strings  ${exitcode}  ${expected_code}  Client return unexpected exit code return: ${exitcode} , but expected: ${expected_code}
    Should Be True  ${canopen}  Cannot Open Keystore/TrustStore by passpshase

Run Cert Service Client Container And Validate Exit Code With Expected API Response
    [Documentation]  Run Cert Service Client Container And Validate Exit Code
    [Arguments]   ${env_file}  ${expected_code}
    ${exitcode}=  Run Client Container  ${DOCKER_CLIENT_IMAGE}  ${CLIENT_CONTAINER_NAME}  ${env_file}  ${CERT_ADDRESS}  ${CERT_SERVICE_NETWORK}
    Remove Client Container And Save Logs  ${CLIENT_CONTAINER_NAME}
