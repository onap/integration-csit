...
Upload VNF Package Content with conflict due to onboarding state
    Log   Upload VNF Package Content with conflict due to onboarding state
	[Documentation]    The objective is to test that the upload of the VNF Package Content fails due to a conflict when the VNF Package is not in onboarding state CREATED in the NFVO
    Create Session    web_session    http://${CATALOG_IP}:${catalog_port}
    ${headers}    Create Dictionary    Accept=application/json
    &{fileParts}=    Create Dictionary
    Create Multi Part   ${fileParts}  file  ${request_csar}
    ...
    ${resp}=  Put Request  web_session  ${vnf_packages_url}/${packageId}/package_content  files=${fileParts}  headers=${headers}
    Log  ${resp.status_code}    
	Should Be Equal As Strings    409    ${resp.status_code}    
