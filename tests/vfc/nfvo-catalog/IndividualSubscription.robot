...
GET Individual VNF Package Subscription with invalid resource identifier
    Log    GET Individual VNF Package Subscription with invalid resource identifier
    [Documentation]    The objective is to test that the retrieval of an individual VNF package subscription fails when using an invalid resource identifier
    ${headers}            Create Dictionary    Content-Type=application/json    Accept=application/json
    Create Session        web_session          http://${CATALOG_IP}:${catalog_port}      headers=${headers}
    ${resp}=              Get Request          web_session     ${subscriptions_url}/invalidId
...
DELETE Individual VNF Package Subscription with invalid resource identifier
    Log   DELETE Individual VNF Package Subscription with invalid resource identifier
    [Documentation]    The objective is to test that the deletion of an individual VNF package subscription fails when using an invalid resource identifier
    ${headers}    Create Dictionary    Content-Type=application/json    Accept=application/json
    Create Session    web_session    http://${CATALOG_IP}:${catalog_port}    headers=${headers}
    ${resp}=    Delete Request    web_session     ${subscriptions_url}/invalidId
    Should Be Equal As Strings    404    ${resp.status_code}
