...
class PrhLibrary(object):
    ...
    @staticmethod
    def find_log_json(prefix, message):
        print("Looking for:")
        ...
        for line in container.logs(stream=True):
            ...
            split = line.split("|")
            if (len(split) > 3):
                match = pattern.match(split[3])
                if match:
                    log_json = match.group(1).replace("\\n", "\n")
                    print("******************Found******************")
                    print(line)
                    print(log_json)
                    print("*****************************************")
                    print(json.loads(log_json))
                    print("*****************")
                    print(json.loads(message))
                    print("*****************************************")
                    if json.loads(log_json) == json.loads(message):
                        return True
        else:
            return False
