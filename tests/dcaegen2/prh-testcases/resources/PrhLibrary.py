...
class PrhLibrary(object):
    ...
    def create_invalid_notification(self, json_file):
        return self.create_pnf_ready_notification_from_ves(json_file).replace("\":", "\": ") \
            .replace("oamV4IpAddress", "ipaddress-v4-oam").replace("oamV6IpAddress", "ipaddress-v6-oam") \
            .replace("serialNumber", "serial-Number").replace("vendorName", "vendor-Name") \
            .replace("modelNumber", "model-Number").replace("unitType", "unit-Type") \
            .replace("}", "\\n}")