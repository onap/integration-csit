...
class PrhLibrary(object):
    ...
    def extract_additional_fields(content, name):
        ...
        # fields = extract_additional_fields(content)
        if fields == []:
            return '"additionalFields":' + 'null'
        # return create_additional_fields_json(fields)
        res = '"' + name + '":{'
        for f in fields:
            res += '"' + f + '"' + ':' + '"' + fields.get(f) + '",'
        return res.rstrip(',') + '},'
    ...
    #     return content.get("event").get("pnfRegistrationFields").get("additionalFields") if "additionalFields" in content["event"]["pnfRegistrationFields"] else []
    #
    # @staticmethod
    # def create_additional_fields_json(fields):
