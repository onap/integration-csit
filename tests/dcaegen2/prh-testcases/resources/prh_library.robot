...
Valid event processing
    [Arguments]    ${input_valid__ves_event_in_dmaap}    ${aai_pnf_registration_simple}
    [Timeout]    30s
    ${data_ves}=    Get Data From File    ${input_valid__ves_event_in_dmaap}
    ${data_aai}=    Get Data From File    ${aai_pnf_registration_simple}
    ${pnf_name}=    Create PNF name    ${data_ves}
    ${aai_content}=    Get Data From File    ${data_aai}
    Set PNF name in AAI    ${pnf_name}
    Set PNF content in AAI    ${aai_content}
    Set event in DMaaP    ${data}
