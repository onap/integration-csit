...
Publish Single VES VNF Measurement Event wit parameter out of schema
    [Tags]    DCAE-VESC-R1
    [Documentation]   Post single event with parameter which is not defined in schema and send to /eventListener/v7 endpoin. Expected 400 Response Status Code
...
Publish PNF Registration Event V7
    [Tags]    DCAE-VESC-R1
    [Documentation]   Post PNF registration event and expect 200 Response Status Code
    Send Request And Validate Response  Publish Event To VES Collector  ${http_session}  ${VES_EVENTLISTENER_V7}  ${EVENT_PNF_REGISTRATION}  202  QTFCOC540002E-reg
