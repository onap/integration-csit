...
${FEED_CONTENT_TYPE}            application/vnd.dr.feed
${SUBSCRIBE_CONTENT_TYPE}       application/vnd.dr.subscription
...
PostCall
    ...
    ${headers}=      Create Dictionary   X-DR-ON-BEHALF-OF=${user}    Content-Type=${content_type}
    ${resp}=         Evaluate            requests.post('${url}', data='${data}', headers=${headers}, verify=False)    requests
    [Return]         ${resp}

PutCall
    [Arguments]      ${url}              ${data}            ${content_type}        ${user}
    ${headers}=      Create Dictionary   X-DR-ON-BEHALF-OF=${user}    Content-Type=${content_type}    Authorization=Basic cnM4NzNtOnJzODczbQ==
...
DeleteCall
    [Arguments]      ${url}              ${user}
    ${headers}=      Create Dictionary   X-DR-ON-BEHALF-OF=${user}
