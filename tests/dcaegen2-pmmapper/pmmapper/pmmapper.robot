...
Verify 3GPP PM Mapper received pushed PM data from data router and publishes to message router.
    ...
    [Timeout]                       1 minute
    SendToDatarouter                ${PM_DATA_FILE_PATH}             ${VALID_METADATA_PATH}            X-ONAP-RequestID=3
...
Verify that PM Mapper logs successful when a file that contains no measdata is provided
    ...
    [Timeout]                       1 minute
    SendToDatarouter                ${NO_MEASDATA_PATH}              ${VALID_METADATA_PATH}            X-ONAP-RequestID=4
...
Verify that PM Mapper throws Event failed validation against schema error when no managed element content is provided
    ...
    [Timeout]                       1 minute
    SendToDatarouter                ${NO_MANAGED_ELEMENT_PATH}       ${VALID_METADATA_PATH}             X-ONAP-RequestID=5
...
Verify that PM Mapper correctly identifies a file that should not be mapped based on metadata filtering.
    ...
    Sleep                           5s
    SendToDatarouter                ${PM_DATA_FILE_PATH}             ${DIFF_VENDOR_METADATA}           X-ONAP-RequestID=6
...
SendToDatarouter
    ...
    ${resp}=                        PutCall                          ${PUBLISH_NODE_URL}        ${request_id}    ${pmdata}    ${metatdata.replace("\n","")}    pmmapper
