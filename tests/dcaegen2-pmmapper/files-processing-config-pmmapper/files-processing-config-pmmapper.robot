...
#    Should Be True                  ${6} < ${dropped_nr}
#    Should Be True                  ${dropped_nr} < ${9}
#    ${isCorrectDroppedCount}=        Run Keyword If  " ${dropped_nr} < ${9}" AND "${dropped_nr} > 6"
#    Log To Console                  ${isCorrectDroppedCount}
    ${result}=                      Evaluate  ${6} < ${dropped_nr} < ${9}
...
#    Run Keyword Unless              ${5} < ${dropped_nr} < ${7}     PrintLogAndFail                               ${alllogs}          ${dropped_nr}
    Should Be True                  ${5} < ${dropped_nr}
    Should Be True                  ${dropped_nr} < ${7}
    ClearLogs
#
...
PrintLogAndFail
    [Arguments]                       ${dmaap_logs}               ${dropped_nr}
    ${pmmapperlogsexec}               Set Variable                docker logs pmmapper
    ${pmmapperlogs}                   Run Process                 ${pmmapperlogsexec}                       shell=yes
    Log To Console                    ${pmmapperlogs.stdout}
    Log To Console                    Number of dropped messages:
    Log To Console                    ${dropped_nr}
    Log To Console                    ${dmaap_logs}
    Fail
