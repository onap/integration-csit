...
Verify that PM Mapper rejects 6-9 messages when limitRate is 1 and threads number is 1
    [Tags]                          FILES_PROCESSING_CONFIG_PM_MAPPER_1
    [Documentation]                 Verify that PM Mapper rejects 6-9/10 messages. Configuration: limitRate=1, threadsNr=1
    [Timeout]                       15 minute
...
Verify that PM Mapper rejects 0 messages when limitRate is 10 and threads number is 1
    [Tags]                          FILES_PROCESSING_CONFIG_PM_MAPPER_2
    [Documentation]                 Verify that PM Mapper rejects 0/10 messages. Configuration: limitRate=10, threadsNr=1
    [Timeout]                       25 minute
    ...
    Sleep                           15s
    SavePmMapperLogsAndDroppedCount  config_10_11  ${dropped_nr}
    Should Be Equal As Numbers      ${dropped_nr}   0   Pm-mapper drop: ${dropped_nr} messages. Expected drop count: 0
    ClearLogs

Verify that PM Mapper rejects 0 messages when limitRate is 1 and threads number is 10
    [Tags]                          FILES_PROCESSING_CONFIG_PM_MAPPER_3
    [Documentation]                 Verify that PM Mapper rejects 0/10 messages. Configuration: limitRate=1, threadsNr=10
    [Timeout]                       25 minute
