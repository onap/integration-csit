...
Get pnf workflow
    ...
    ${workflow_uuid}=          Set Variable        "b2fd5627-55e4-4f4f-8064-9e6f443e9152"
    #${pnfworkflows_json_response}[workflowSpecificationList][workflowSpecification][artifactInfo][artifactUuid]
...
Invoke Service Instantiation
    ...
    ${workflow_uuid}=        Set Variable    "4752c287-c5a8-40a6-8fce-077e1d54104b"
...
Test AAI-update for target software version
    ...
    \   Run Keyword If  '${sw_version}' == 'pnf_sw_version-2.0.0'  log to console      Exit For Loop
    \   log to console  Will try again after ${SLEEP_INTERVAL_SEC} seconds
    \   SLEEP   ${SLEEP_INTERVAL_SEC}s

    Log To Console     final target software version received: ${sw_version}
    Run Keyword If  '${sw_version}' == 'pnf_sw_version-2.0.0'  log to console   \nexecuted with expected result
    Should Be Equal As Strings    '${sw_version}'    'pnf_sw_version-2.0.0'
