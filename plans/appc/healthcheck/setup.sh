...
TIME=0
while [ "$TIME" -lt "$TIME_OUT" ]; do
  response=$(docker exec -i appc_controller_container /opt/opendaylight/current/bin/client system:start-level)
...
TIME=0
while [ "$TIME" -lt "$TIME_OUT" ]; do
  response=$(docker exec -i appc_controller_container /opt/opendaylight/current/bin/client bundle:list | grep appc-design-services-provider)
