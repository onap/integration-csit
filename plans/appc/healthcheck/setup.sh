...
export APPC_DOCKER_IMAGE_VERSION=1.7.1
...
TIME=0
while [ "$TIME" -lt "$TIME_OUT" ]; do
  response=$(docker exec -it appc_controller_container /opt/opendaylight/current/bin/client system:start-level)
...
TIME=0
while [ "$TIME" -lt "$TIME_OUT" ]; do
  response=$(docker exec -it appc_controller_container /opt/opendaylight/current/bin/client bundle:list | grep appc-design-services-provider)
