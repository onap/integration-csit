...
# Tested variants of REST/DMAAP config
TESTED_VARIANTS="REST   DMAAP  REST+SDNC"
#Test agent and simulator protocol variants
TESTED_PROTOCOLS="HTTPS"
...
for __httpx in $TESTED_PROTOCOLS ; do
    for interface in $TESTED_VARIANTS ; do
        ...
        if [ "$PMS_VERSION" == "V2" ]; then
            ...
            cr_equal received_callbacks?id=ric-registration 3 120
        ...
        if [ "$PMS_VERSION" == "V2" ]; then
            notificationurl=$CR_SERVICE_PATH/2
        else
            notificationurl=""
        fi
        ...
        if [ "$PMS_VERSION" == "V2" ]; then
            cr_equal received_callbacks?id=ric-registration 5
        fi
