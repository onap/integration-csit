...
res=$($curlString)
echo "$res"
expect1="Policy type 1 is OK.201"
expect2="The policy type already exists and instances exists400"
expect3="Policy type 1 is OK.200"
if [ "$res" != "$expect1" -a "$res" != "$expect2" -a "$res" != "$expect3" ]; then
   echo "$res is not expected! exit!"
   exit 1;
fi
echo -e "\n"
...
res=$($curlString)
echo "$res"
expect1="Policy type 2 is OK.201"
expect2="The policy type already exists and instances exists400"
expect3="Policy type 2 is OK.200"
if [ "$res" != "$expect1" -a "$res" != "$expect2" -a "$res" != "$expect3" ]; then
   echo "$res is not expected! exit!"
   exit 1;
fi
...
expect1="200"
expect2="201"
if [ "$res" != "$expect1" -a "$res" != "$expect2" ]; then
   echo "$res is not expected! exit!"
   exit 1;
fi
echo -e "\n"
