...
PROJECT_DIRECTORY="plans/oom/certservice"
...
mkdir -p ${WORKSPACE}/tests/oom/certservice/assets/certs/
make all -C ./certs/
cp ${WORKSPACE}/plans/oom/certservice/certs/root.crt ${WORKSPACE}/tests/oom/certservice/assets/certs/root.crt
echo "Generated keystores"
openssl pkcs12 -in ${WORKSPACE}/plans/oom/certservice/certs/certServiceServer-keystore.p12 -clcerts -nokeys -password pass:secret | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ${WORKSPACE}/tests/oom/certservice/assets/certs/certServiceServer.crt
echo "Generated server certificate"
openssl pkcs12 -in ${WORKSPACE}/plans/oom/certservice/certs/certServiceServer-keystore.p12 -nocerts -nodes -password pass:secret| sed -ne '/-BEGIN PRIVATE KEY-/,/-END PRIVATE KEY-/p' > ${WORKSPACE}/tests/oom/certservice/assets/certs/certServiceServer.key
