...
CONFIG_DIR_MACRO=$MACRO_HOME/config
ENV_FILE=$CONFIG_DIR_MACRO/env
DOCKER_COMPOSE_FILE_PATH=$MACRO_HOME/docker-compose.yml
DOCKER_COMPOSE_LOCAL_OVERRIDE_FILE=$MACRO_HOME/docker-compose.local.yml
TEAR_DOWN_SCRIPT=$MACRO_HOME/teardown.sh
...
docker ps -a
docker logs $(docker container ls -a| grep sdc-controller | awk '{ print $1 }')
