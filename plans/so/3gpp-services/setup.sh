...
SIMULATORS=$WORKSPACE/scripts/so/3gpp-simulators/
...
MVN_CLEAN_INSTALL="$MVN clean install -Dmaven.test.skip"
...
cd $SIMULATORS
pwd
mvn -f oof-simulator/pom.xml clean install
docker build -t simulators/oof-simulator oof-simulator/

mvn -f  kafkaListener/pom.xml clean install
docker build -t simulators/kafkalistener kafkaListener/
cd $SCRIPT_HOME
pwd 
...
docker cp config/subnetCapability.json 3gppservices_api-handler-infra_1:/app

#DMAAP setup

DMAAP_IP=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' 3gppservices_dmaap_1)
