...
# Source SDNC, AAF-CertService, Netconf-Pnp-Simulator config enviroment file
...
if [[ "${AAFCERT_IP}" == 'none' || "${AAFCERT_IP}" == '' && "${RESP_CODE}" != '200' ]]; then
...
echo "Sleeping additional for 5 minutes to give application time to finish"
sleep 5m
