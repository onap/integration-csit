...
sed -i .bak "s/^LATITUDE=.*/LATITUDE=${LATITUDE:=38.0}/" d.props
sed -i .bak "s/^LONGITUDE=.*/LONGITUDE=${LONGITUDE:=-72.0}/" d.props
sed -i .bak "s/NEXUS_DOCKER_REPO=.*/NEXUS_DOCKER_REPO=${NEXUS_DOCKER_REPO:=nexus3.onap.org:10001}/"  d.props
sed -i .bak "s/HOSTNAME=.*/HOSTNAME=aaf.api.simpledemo.onap.org/"  d.props
if [ "$(docker system info | grep 'Name:')" = "Name: minikube" ]; then
  echo "Minikube IP"
  HOST_IP=$(minikube ip)
else 
  HOST_IP=$(ip route get 8.8.8.8 | awk '{print $3}')
fi
sed -i .bak "s/HOST_IP=.*/HOST_IP=$HOST_IP/" d.props
