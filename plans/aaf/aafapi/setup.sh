...
if [ -e "/archives/opt/authz" ]; then
  cd /archives/opt/authz
  git pull
else
  cd /archvies/opt
...
if [ ! -e /archvies/opt/authz/auth/docker/d.props ]; then
  cp /archvies/opt/authz/auth/docker/d.props.init /archvies/opt/authz/auth/docker/d.props
fi
...
NEXUS_PASSWD=anonymous
NEXUS_DOCKER_REPO=nexus3.onap.org:10001
sed -i "s/DOCKER_REPOSITORY=.*/DOCKER_REPOSITORY=$NEXUS_DOCKER_REPO/" /archvies/opt/authz/auth/docker/d.props
. /archvies/opt/authz/auth/docker/d.props
...
cd /archvies/opt/authz/auth/auth-cass/docker
...
CASS_IP=`docker inspect aaf_cass | grep '"IPAddress' | head -1 | cut -d '"' -f 4`
CASS_HOST="cass.aaf.osaaf.org:"$CASS_IP
if [ ! -e /archvies/opt/authz/auth/docker/cass.props ]; then
  cp /archvies/opt/authz/auth/docker/cass.props.init /archvies/opt/authz/auth/docker/cass.props
fi

sed -i "s/CASS_HOST=.*/CASS_HOST="$CASS_HOST"/g" /archvies/opt/authz/auth/docker/cass.props
# TODO Pull from Config Dir
if [ "$LATITUDE" = "" ]; then
  LATITUDE=37.781
  LONGITUDE=-122.261
  sed -i "s/LATITUDE=.*/LATITUDE=$LATITUDE/g" /archvies/opt/authz/auth/docker/d.props
  sed -i "s/LONGITUDE=.*/LONGITUDE=$LONGITUDE/g" /archvies/opt/authz/auth/docker/d.props
fi

sed -i "s/VERSION=.*/VERSION=$VERSION/g" /archvies/opt/authz/auth/docker/d.props
sed -i "s/HOSTNAME=.*/HOSTNAME=$HOSTNAME/g" /archvies/opt/authz/auth/docker/d.props
sed -i "s/HOST_IP=.*/HOST_IP=$HOST_IP/g" /archvies/opt/authz/auth/docker/d.props
sed -i "s/AAF_REGISTER_AS=.*/AAF_REGISTER_AS=$FQDN/g" /archvies/opt/authz/auth/docker/d.props

cd /archvies/opt/authz/auth/docker
...
for i in {1..12}; do
   curl -k -u aaf_admin@people.osaaf.org:demo123456! https://${AAF_IP}:8100/authz/roles/user/demo@people.osaaf.org && break
    echo sleep $i
    sleep $i
done
