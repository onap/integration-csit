...
echo "This is ${WORKSPACE}/scripts/clamp/start_cmso_containers.sh"
...
# WAIT 5 minutes maximum and test every 5 seconds if cmso up using HealthCheck API
#TIME_OUT=600
#INTERVAL=5
#TIME=0
#while [ "$TIME" -lt "$TIME_OUT" ]; do
#  response=$(curl --write-out '%{http_code}' --silent --output /dev/null -vk --key config/org.onap.clamp.keyfile https://localhost:8080/cmso/v1/health); echo $response
