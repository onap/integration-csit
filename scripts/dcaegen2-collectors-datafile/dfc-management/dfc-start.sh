...
for i in {1..10}; do
  if [ $(docker inspect --format '{{ .State.Running }}' $DFC_APP) ]
    then
      ...
      docker exec $DFC_APP /bin/rm /opt/app/datafile/config/datafile_endpoints.json
      docker exec $DFC_APP /bin/rm config/datafile_endpoints.json
...
echo "ERROR: could not start DFC"
exit 1
