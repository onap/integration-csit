...
if [ "$#" -lt 2 ]; then
    ...
    echo "Needs gerrit branch name and atleast one project name."
    exit -1
fi
...
git clone -b $GERRIT_BRANCH "https://gerrit.onap.org/r/policy/${PROJ}"
cd ${PROJ}
PROJECT_VERSIONS+=($(printf 'VERSION=${project.version}\n0\n' | mvn org.apache.maven.plugins:maven-help-plugin:2.1.1:evaluate | grep '^VERSION' | cut -d "=" -f 2))
#Display full array every time.
echo "${PROJECT_VERSIONS[@]}"
cd .. # Go back to work dir
done
