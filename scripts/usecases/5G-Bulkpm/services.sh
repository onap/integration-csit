...
cd /tmp/ || exit
echo "Checking installed PM Mapper and DFC services..if not installed, script will install.."
mkdir blueprints
PMMAPPER_POD=$(kubectl -n onap get pods | sed 's/ .*//'| grep dcae-pm-mapper)
if [ -z "${PMMAPPER_POD}" ]
then
    echo "Installing PM Mapper..."
    cd /tmp/blueprints || exit
    ...
    kubectl -n onap cp input.yaml "${POD}":/root/
    kubectl -n onap cp k8s-pm-mapper.yaml "${POD}":/root/
    kubectl -n onap exec -it "${POD}" -- cfy install --blueprint-id pmmapper --deployment-id pmmapper -i /root/input.yaml /root/k8s-pm-mapper.yaml
...
then
    ...
    kubectl -n onap cp k8s-datafile.yaml "${POD}":/root/
    kubectl -n onap exec -it "${POD}" -- cfy install --blueprint-id dfc --deployment-id dfc /root/k8s-datafile.yaml
